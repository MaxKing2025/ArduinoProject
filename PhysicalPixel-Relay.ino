...
void loop() {
  ...
  if (Serial.available() > 0) {
   ...
   // if it's a capital P (ASCII 72), turn on the LED:
    ...
    // if it's an M (ASCII 76) turn off the LED:
    if (incomingByte == 'M') {
      digitalWrite(ledPin2, LOW);
    }
    // if it's an Q (ASCII 76) turn on the LED:
    if (incomingByte == 'Q') {
      digitalWrite(ledPin3, HIGH);
    }
    // if it's an W (ASCII 76) turn off the LED:
    if (incomingByte == 'W') {
      digitalWrite(ledPin3, LOW);
    }
     // if it's an Z (ASCII 76) turn on the LED:
    if (incomingByte == 'Z') {
      digitalWrite(ledPin1, HIGH);
    }
    // if it's an X (ASCII 76) turn off the LED:
    if (incomingByte == 'X') {
